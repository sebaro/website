...
function process(method) {
  ...
  if (method == 'add') {
    ...
    db[food] = {};
    db[food]['weight'] = parseInt(weight);

    var name, unit, value;
    for (var i = 0; i < data.length; i++) {
      name = data[i]['name'];
      unit = data[i]['unit'].toLowerCase();
      value = data[i]['value'];
    ...
    for (var vitamin in nutrientsIU['Vitamins']) {
      if (!db[food][vitamin] && db[food][nutrientsIU['Vitamins'][vitamin]]) {
        value = db[food][nutrientsIU['Vitamins'][vitamin]];
        value = parseFloat((db[food][nutrientsIU['Vitamins'][vitamin]] * 0.3).toFixed(3));
        value = value.toString().replace(/0+$/, '');
        value = value.toString().replace(/\.$/, '');
        if (value.toString().indexOf('.') == -1) {
          value = parseInt(value);
...
function show() {
  ...
  cell.className = 'total_header';
  table.appendChild(cell);
  cell = document.createElement('div');
  cell.innerHTML = db['total']['weight'] + 'g';
  cell.className = 'total_weight';
  table.appendChild(cell);
  nl = document.createElement('br');
  table.appendChild(nl);
