...
function add(ndbno) {
  ...
  var url = 'http://api.nal.usda.gov/ndb/reports/?type=f&format=json&api_key=' + api_key + '&ndbno=' + ndbno;
...
function show() {
  ...
  for (var ng in nutrients) {
    ...
    for (var n in nutrients[ng]) {
      ...
      // uvalue = adequate - (adequate * 40/100)
      // ovalue = adequate - (adequate * 20/100)
      // 0 (red) uvalue (yellow) ovalue (green) tolerable (purple)
      avalue = intake[ng][n][stage]['adequate'];
      if (avalue < 0) avalue = 0;
      tvalue = intake[ng][n][stage]['tolerable'];
      if (tvalue < 0) tvalue = 0;
      uvalue = avalue - (avalue * 40 / 100);
      ovalue = avalue - (avalue * 20 / 100);
      if (cvalue < uvalue) {
	cell.className = 'nutrient_' + ng.toLowerCase() + ' nutrient_alert';
      }
      else if (cvalue >= uvalue && cvalue < ovalue) {
	cell.className = 'nutrient_' + ng.toLowerCase() + ' nutrient_warning';
      }
      else if (cvalue >= ovalue && cvalue <= tvalue) {
	cell.className = 'nutrient_' + ng.toLowerCase() + ' nutrient_good';
      }
      else {
	cell.className = 'nutrient_' + ng.toLowerCase() + ' nutrient_alert2';
      }
      table.appendChild(cell);
    }
    nl = document.createElement('br');
